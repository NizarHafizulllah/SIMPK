<?php 
class admin_add_pengepul extends admin_controller{
	var $controller;
	function admin_add_pengepul(){
		parent::__construct();

		$this->controller = get_class($this);
		$this->load->model('admin_pengepul_model','dm');
...
function index(){
		$data_array=array();
		$content = $this->load->view("admin_pengepul_view",$data_array,true);

		$this->set_subtitle("Tambah Pengepul");
		$this->set_title("Tambah Pengepul");
...
if($this->form_validation->run() == TRUE ) { 
        ...
        $res = $this->db->insert('super_admin', $post); 
        if($res){
